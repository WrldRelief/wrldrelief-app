...
import { Wallet, Coins } from "iconoir-react";

export const TransactionConfirm: React.FC = () => {
  const { donationState, goToStep, setPaymentMethod, setSelectedToken, processWorldAppPayment } = useDonation();
  const [paymentTab, setPaymentTab] = useState<'standard' | 'worldapp'>(donationState.paymentMethod);
  const [selectedToken, setTokenSelection] = useState<TokenType>(donationState.selectedToken);
  ...
  return (
    <div className="w-full max-w-md mx-auto bg-white rounded-lg shadow-md overflow-hidden">
      ...
      <div className="p-6">
        ...
        {paymentTab === 'worldapp' && (
          <div className="mb-6">
            <h3 className="text-sm font-medium text-gray-700 mb-3">Select Token</h3>
            <div className="grid grid-cols-2 gap-3">
              <div 
                onClick={() => handleTokenChange('USDC')}
                className={`flex items-center p-3 border rounded-lg cursor-pointer transition-all ${selectedToken === 'USDC' ? 'border-black bg-gray-50' : 'border-gray-200 hover:bg-gray-50'}`}
              >
                <div className="w-6 h-6 mr-2">
                  <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                    <circle cx="12" cy="12" r="12" fill="#2775CA"/>
                    <path d="M15.6 12C15.6 10.01 14.09 8.4 12.1 8.4C10.11 8.4 8.6 10.01 8.6 12C8.6 13.99 10.11 15.6 12.1 15.6C14.09 15.6 15.6 13.99 15.6 12Z" fill="white"/>
                    <path d="M7.8 9.75V14.25L12 16.5L16.2 14.25V9.75L12 7.5L7.8 9.75Z" stroke="white" strokeWidth="0.5" strokeLinecap="round" strokeLinejoin="round"/>
                  </svg>
                </div>
              ...
              <div 
                ...
                className={`flex items-center p-3 border rounded-lg cursor-pointer transition-all ${selectedToken === 'WLD' ? 'border-black bg-gray-50' : 'border-gray-200 hover:bg-gray-50'}`}
              >
                <div className="w-6 h-6 mr-2">
                  <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
